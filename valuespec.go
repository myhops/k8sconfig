...
func (v *valueSpec) validate() error {
	...
	if len(v.namespace) == 0 {
		return fmt.Errorf("namespace is mandatore")
	}

	// validate the name length
	if len(v.name) > 253 {
		return fmt.Errorf("name should not be longer than 253 characters")
	}
	// validate the name
...
func (v *valueSpec) validateConfigMap() error {
	...
	case "data", "binaryData":
		break
	default:
		return fmt.Errorf("configMap: need data or binaryData")
	}
...
func (v *valueSpec) validateSecret() error {
	...
	case "data", "stringData":
		break
	default:
		return fmt.Errorf("secret: need data or strintData")
	}
